...
class Agent(threading.Thread):
    ...
    def has_received_a_move(self):
        if self.id in message_box and len(message_box[self.id]) != 0:
            for m in message_box[self.id]:
                if m.x == self.x and m.y == self.y:
                    return True
        return False

    def find_another_position(self):
        possibilities = []
        for tx, ty in directions:
            if self.y + ty > self.board.n-1 or self.y+ty < 0 or self.x + tx < 0 or self.x + tx > self.board.n - 1:
                continue
            if self.board.occupiedCase[self.y + ty][self.x + tx] > 0:
                continue
            possibilities.append([tx, ty])
...
while True:
    ...
    pygame.draw.rect(screen, BLACK, (windowWidth + 30 + 240 * ((error_rate + sliderMinError) / sliderMaxError) - 15, 320, 30, 40))
    ...
    if pygame.mouse.get_pressed()[0]:
        ...
        if pos[0] > windowWidth:
            if pos[1] < 100:
                ...
                i_move = max(1, int(((max(windowWidth + 30, pos[0]) - (windowWidth + 30) + sliderMinI) / 240) * sliderMaxI))
            elif pos[1] < 400:
                error_rate = (((max(windowWidth + 30, pos[0]) - (windowWidth + 30) + sliderMinError) / 240) * sliderMaxError)
